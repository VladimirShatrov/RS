...
namespace SocketTcpServer
{
    class Server
    {
                ...
                while (true)
                {
                    ...
                    try
                    {
                        while (true)
                        {
                            ...
                            {
                                StopConnection(handler, listenSocket);
                                break;
                            }
                            else
                            {
                    ...
                    {
                        HardStopConnection(handler, listenSocket);
                    }
                }
            }
            catch (Exception ex)
        ...
        private static void StopConnection(Socket handler, Socket listenSocket)
        {
        ...
        private static void HardStopConnection(Socket handler, Socket listenSocket)
        {
            handler.Shutdown(SocketShutdown.Both); // отключение передачи и приема на сокет
            handler.Close(); // закрываем сокет
        }
